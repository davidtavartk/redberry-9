...
const CustomButton = forwardRef<HTMLButtonElement, ButtonProps>((props, forwardedRef) => {
  ...
  return (
    <button
      ...
      {...otherProps}
      ref={internalRef}
      className={`text-c-grey flex cursor-pointer items-center justify-center rounded-[5px] border px-5 py-2.5 transition-all duration-200 ease-in-out ${filled ? "bg-[#8338EC] text-white hover:bg-[#8869b4] active:bg-[#6e4ba0]" : "border border-[#8338EC] hover:bg-[#883af5] hover:text-white active:bg-[#9157e4]"} ${className}`}
      type={type}
    >
